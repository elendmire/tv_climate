...
def test_rolling_window_trims_to_30(tmp_path: Path) -> None:
    p = tmp_path / "roll.json"
    base = datetime(2026, 1, 1, 12, 0, tzinfo=timezone.utc)
    one = [_item("https://a/x", "s1", True)]
    for i in range(35):
        merge_daily_timeseries(p, one, base + timedelta(days=i), max_days=ROLLING_WINDOW_DAYS)
    data = json.loads(p.read_text(encoding="utf-8"))
    assert len(data["series"]) == 30
    assert data["series"][0]["date"] == "2026-01-06"
